  ...
  useEffect(() => {
    const fetchRealEstates = async () => {
      try {
        ...
        console.error("Firebase'den emlak verileri alınamadı:", error);
      }
    };
    fetchRealEstates();
  }, []);
    ...
    <SafeAreaView style={{backgroundColor: renkler.white, flex: 1}}>
      ...
      <View style={style.header}>
        <View>
          ...
          <Text style={{color: renkler.dark, fontSize: 20, fontWeight: 'bold'}}>
            Türkiye
          </Text>
        </View>
        <Image
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <FlatList
          ...
          horizontal
          data={realEstates} // Firestore'dan gelen emlak verilerini kullan
          renderItem={({item}) => <Card house={item} />} // Her bir öğe için Card bileşenini oluştur
        />
      </ScrollView>
    </SafeAreaView>
  );
